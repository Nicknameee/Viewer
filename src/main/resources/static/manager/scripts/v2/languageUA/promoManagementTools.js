...
function processType(element) {
    if ($(element).find(":selected").text() === 'Оберіть тип') {
        $(element).parent().addClass('valid-dec')
        $(element).parent().attr('about' , 'Потрібно обрати тип')
                ...
                success:
                    function (response) {
                        if (response.success) {
                            ...
                            $("#add-promo-sec-title").attr('about' , 'Промо було успішно створено')
                            setTimeout(function () {
...
async function deletePromo(element) {
    let confirmation = confirm("Ви впевнені щодо видалення даного промо?")
    if (confirmation) {
        let sessionValid = await checkSessionValidity()
        if (sessionValid) {
            $.ajax({
