...
function processBank(element , type) {
    if ($(element).find(":selected").text() === 'Оберіть банк') {
        $(element).parent().addClass('valid-dec')
        $(element).parent().attr('about' , 'Потрібно обрати банк')
        if (type === 'add') {
            canConfirmAdd = false
        }
        if (type === 'edit') {
                    ...
                    success:
                        function(response) {
                            if (response.success) {
                                ...
                                $("#add-payment-sec-title").attr('about' , 'Картку було успішно додано')
                                setTimeout(function () {
                                ...
                                $("#add-payment-sec-title").attr('about' , 'Під час додавання картки виникла помилка')
                    ...
                    success:
                        function(response) {
                            if (response.success) {
                                ...
                                sendAlertRequestHomePage()
                                $(element).parent().parent().parent().children().eq(0).attr('about' , 'Дані про картку було оноловнено успішно')
                                setTimeout(function () {
                                    $("#add-payment-sec-title").attr('about' , '');
                                    let link = window.location.protocol + window.location.host + window.location.pathname;
                                    let url = new URL(link);
                                ...
                                $("#add-payment-sec-title").attr('about' , 'Під час оновлення картки виникла помилка')
...
async function deletePayment(element) {
    let conf = confirm("Ви впевнені щодо видалення даної картки?")
    if (conf) {
        let sessionValid = await checkSessionValidity()
        if (sessionValid) {
            $.ajax(
