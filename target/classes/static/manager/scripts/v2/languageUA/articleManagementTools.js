...
function addFile(element , type , action) {
    if (!validate_files(element)) {
        $(element).parent().addClass('valid-dec')
        $(element).parent().attr('about' , 'Максимальний розмір файлу 250MB')
        if (type === 'add') {
            canSubmitAdd = false
        }
        if (type === 'edit') {
                ...
                success:
                    function(response) {
                        ...
                        if (response.success) {
                            ...
                            $(element).parent().parent().prev().attr('about' , 'Файл було видалено успішно')
                            setTimeout(function () {$(element).parent().parent().prev().attr('about' , ''); $(element).parent().remove()} , 3000)
                        }
                        else {
                            $(element).parent().parent().prev().attr('about' , 'Виникла помилка під час видалення файлу')
                    ...
                    success:
                        function(response) {
                            ...
                            if (response.success) {
                                ...
                                $(element).parent().parent().parent().children().eq(0).attr('about' , 'Новина була створена успішно')
                                setTimeout(function () {$(element).parent().parent().parent().children().eq(0).attr('about' , ''); location.reload()} , 2000)
                            }
                            else {
                                $(element).parent().parent().parent().children().eq(0).attr('about' , 'Виникла помилка під час створення новини')
                    ...
                    success:
                        function (response) {
                            ...
                            if (response.success) {
                                ...
                                sendAlertRequestArticlePage(artSecret)
                                sendAlertRequestHomePage()
                                $(element).parent().parent().parent().children().eq(0).attr('about', 'Новина оновлена успішно')
                                setTimeout(function () {
                                    $(element).parent().parent().parent().children().eq(0).attr('about', '');
                                    location.reload()
                                }, 2000)
                            } else {
                                $(element).parent().parent().parent().children().eq(0).attr('about', 'Під час оновлення новини виникла помилка')
                                setTimeout(function () {
                                    $(element).parent().parent().parent().children().eq(0).attr('about', '')
                                }, 3000)
                                console.log(response.error)
...
async function deleteArticle(element) {
    let confirmation = confirm("Ви впевнені щодо видалення даної новини?")
    if (confirmation) {
        let sessionValid = await checkSessionValidity()
        if (sessionValid) {
